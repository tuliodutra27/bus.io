...
Regra do modelo híbrido:
    ...
    ocupação(ônibus, data) = AlocacaoFixa
                             − ausências(data)
...
def montar_mapa(db: Session, onibus: Onibus, dia: date) -> MapaView:
    ...
    # Roster fixo: assento_id -> colaborador
    fixos = {
    ...
    for a in assentos:
        ...
        # Extra (hora extra / eventual) tem precedência de exibição no dia
    ...
    return MapaView(
        onibus=onibus, assentos=views, total=len(views), ocupados=ocupados
    )
...
def onibus_turno_da_rota(db: Session, rota_id: int) -> Optional[Onibus]:
    ...
    from app.models import TipoOnibus
